...
from src.providers.openai_compatible import CallbackType, OpenAICompatibleProvider
...
class TestStreamingRetry(unittest.TestCase):
    ...
    def test_openai_429_retry(self, mock_sleep, mock_post):
        ...
        result = provider.generate_stream(
            messages=[{"role": "user", "content": "hi"}], model="test-model", params={}, callback=callback
        )
    ...
    def test_openai_empty_response_retry(self, mock_sleep, mock_post):
        ...
        mock_response_empty.iter_lines.return_value = ['data: {"usage": {"completion_tokens": 0}}', "data: [DONE]"]
    ...
    def test_gemini_network_error_retry(self, mock_sleep, mock_post):
        ...
        result = provider.generate_stream(
            messages=[{"role": "user", "content": "hi"}], model="gemini-2.0-flash", params={}, callback=callback
        )
    ...
    def test_openai_thinking_extraction_streaming(self, mock_sleep, mock_post):
        ...
        callback_data = {CallbackType.THINKING: [], CallbackType.TEXT: [], CallbackType.USAGE: None}
    ...
    def test_openai_thinking_extraction_nonstreaming(self, mock_sleep, mock_post):
        ...
        mock_response.json.return_value = {
            "choices": [
                {"message": {"content": "Here's my answer", "reasoning_content": "First I analyzed... then concluded"}}
            ],
        ...
        result = provider.generate(
            messages=[{"role": "user", "content": "Explain"}], model="test-model", params={}, thinking_enabled=True
        )
    ...
    def test_gemini_thinking_extraction_streaming(self, mock_sleep, mock_post):
        ...
        # Track callbacks
        callback_data = {CallbackType.THINKING: [], CallbackType.TEXT: []}
    ...
    def test_gemini_thinking_extraction_nonstreaming(self, mock_sleep, mock_post):
        ...
        result = provider.generate(
            messages=[{"role": "user", "content": "Decide"}], model="gemini-2.5-flash", params={}, thinking_enabled=True
        )
    ...
    def test_thinking_config_in_request_body_openai(self, mock_sleep, mock_post):
        ...
        provider = OpenAICompatibleProvider("custom", "http://fake.url", self.key_manager, {"reasoning_effort": "high"})
    ...
    def test_gemini_3_uses_thinking_level(self, mock_sleep, mock_post):
        ...
        provider = GeminiNativeProvider(self.key_manager, {"thinking_level": "high"})
    ...
    def test_openai_inline_thinking_extraction_nonstreaming(self, mock_sleep, mock_post):
        ...
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "choices": [{"message": {"content": "<think>Let me analyze</think>Final conclusion"}}],
            "usage": {"prompt_tokens": 10, "completion_tokens": 20, "total_tokens": 30},
        }
        mock_post.return_value = mock_response

        result = provider.generate(messages=[{"role": "user", "content": "hi"}], model="test-model", params={})
