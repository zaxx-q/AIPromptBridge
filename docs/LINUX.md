...
- **Terminal-safe copy:** When the focused window is a terminal emulator, TextEdit automatically uses **Ctrl+Shift+C** instead of Ctrl+C to avoid sending SIGINT to foreground processes. Terminal detection uses compositor IPC (`niri msg`, `swaymsg`, `hyprctl`) to read the focused window's `app_id`. Configure via `text_edit_terminal_copy_shortcut` in `config.ini`: `auto` (default, detect terminals), `always_ctrl_c`, or `always_ctrl_shift_c`. Compare Mode also accepts both Ctrl+C and Ctrl+Shift+C.
