...
class ToolsTabMixin:
    ...
    def _create_tools_tab(self, frame):
        ...
        self._add_toggle_field(content, "text_edit_tool_enabled",
                               "Enable TextEditTool",
                               self.config_data.config.get("text_edit_tool_enabled", True),
                               hint="⚠️ Restart required")

        self._add_entry_field(content, "text_edit_tool_hotkey", "Activation hotkey:",
                             self.config_data.config.get("text_edit_tool_hotkey", "ctrl+space"),
                             size="md", hint="⚠️ Restart required")

        self._add_entry_field(content, "text_edit_tool_abort_hotkey", "Abort hotkey:",
        ...
        self._add_toggle_field(content, "screen_snip_enabled",
                               "Enable ScreenSnip",
                               self.config_data.config.get("screen_snip_enabled", True),
                               hint="⚠️ Restart required")

        self._add_entry_field(content, "screen_snip_hotkey", "ScreenSnip hotkey:",
        ...
        self._add_toggle_field(content, "audio_tool_enabled",
                               "Enable Audio Tool",
                               self.config_data.config.get("audio_tool_enabled", True),
                               hint="⚠️ Restart required")

        self._add_entry_field(content, "audio_tool_hotkey", "Audio Tool hotkey:",
                             self.config_data.config.get("audio_tool_hotkey", "ctrl+alt+a"),
                             size="md", hint="⚠️ Restart required")

        self._add_entry_field(content, "audio_default_device", "Default device:",
                             self.config_data.config.get("audio_default_device") or "default",
                             size="lg",
                             hint="Partial name match ('default' = system default)")

        self._add_toggle_field(content, "audio_default_loopback",
        ...
        self._add_dropdown_field(content, "audio_level_meter_style", "Level meter style:",
