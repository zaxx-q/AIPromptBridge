...
class AudioToolApp:
    ...
    def _on_action_selected(
        ...
        """
        logging.debug(
            f"Action selected: key={action_key}, custom={bool(custom_input)}, duration={duration:.1f}s, compressed={compressed}"
        )
            ...
            try:
                ...
                if not messages:
                    ...
                    messages = build_audio_message(
                        audio_b64=audio_b64, mime_type=mime_type, task=task, system_prompt=system_prompt
                    )
    ...
    def _process_action(
        ...
        try:
            ...
            show_error_popup(
                title="Audio Tool Error", message=f"Failed to process '{action_key}' action.", details=str(e)
            )
        finally:
            self._end_task()
    ...
    def _stream_to_chat_window(
        ...
        attachment_path = AttachmentManager.save_audio(
            session_id=session.session_id, audio_data=audio_data, mime_type=mime_type, message_index=0
        )
        attachments = []
        if attachment_path:
            attachments.append({"path": attachment_path, "mime_type": mime_type, "duration": duration})
        ...
        if isinstance(user_content, list):
            task_text = next(
                (item["text"] for item in user_content if item.get("type") == "text"), "Analyze this audio."
            )
        else:
        ...
        if streaming_enabled:
            ...
            if ctx.error:
                ...
                show_error_popup(title="API Request Failed", message="Failed to analyze audio.", details=ctx.error)
                return
