...
- **Popup position & focus:** Hyprland cursor IPC places popups near the visible cursor. On compositors without cursor IPC (niri, Sway), popups automatically center on the currently focused monitor via compositor output queries (avoiding stale Xwayland cursor traps from apps like ONLYOFFICE). Popups remain unmanaged override-redirect windows so dynamic content changes and carousel navigation do not incur compositor configure-transaction latency.
