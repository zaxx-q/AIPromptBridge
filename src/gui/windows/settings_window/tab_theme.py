...
class ThemeTabMixin:
    ...
    def _create_theme_tab(self, frame):
        ...
        self._add_dropdown_field(content, "ui_theme", "Theme:",
                                 ...
                                 options=list_themes(), size="md",
                                 command=lambda x: self._update_theme_preview())

        self._add_dropdown_field(content, "ui_theme_mode", "Mode:",
        ...
        self._add_entry_field(content, "chat_user_bg_color", "User msg background:",
                             self.config_data.config.get("chat_user_bg_color") or "",
                             size="sm",
                             hint="Hex color override (e.g. #1e3a5f). Empty = theme default")

        self._add_entry_field(content, "chat_assistant_bg_color", "Assistant msg background:",
