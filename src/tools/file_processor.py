...
class FileProcessor(BaseTool):
    ...
    def _customize_effect_params(self, effect_name: str, default_params: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if effect_name == "highpass":
            print("  Cutoff frequency [80] Hz: ", end="")
            try:
                val = input().strip()
                if val:
                    return {"f": int(val)}
        ...
        if effect_name == "lowpass":
            print("  Cutoff frequency [12000] Hz: ", end="")
            try:
                val = input().strip()
                if val:
                    return {"f": int(val)}
        ...
        if effect_name == "afftdn":
            print("  Noise reduction strength (0-97) [15]: ", end="")
            try:
                val = input().strip()
                if val:
        ...
        if effect_name == "equalizer":
            print("  Center frequency [3000] Hz: ", end="")
            try:
                freq = input().strip()
                print("  Gain [-12 to +12] [3] dB: ", end="")
                gain = input().strip()
    ...
    def _preview_file_size(self, audio_files: List[FileInfo], current_config: Dict[str, Any]):
        ...
        if preprocess_type == "preset":
            ...
            print("    • Normalize: EBU R128")

        if convert_to_mono:
            print("    • Channels: Convert to mono")
        ...
        if not any([preprocess_type, convert_to_mono, target_sample_rate, target_bitrate]):
            print("    • (No changes - original format)")
    ...
    def _step_file_optimization(
        ...
        if not optimization.sample_rate:
            print("  ✓ Keeping original sample rate")
        ...
        if not optimization.bitrate_kbps:
            print("  ✓ Using codec default bitrate")
    ...
    def _step_output_configuration(self, scan_result: ScanResult, prompt_key: str) -> Optional[Dict[str, Any]]:
        ...
        if output_mode == "individual":
            ...
            naming = "batch_output_{date}_{time}"
            ...
            for i, file_path in enumerate(remaining):
                ...
                try:
                    ...
                    if cp.output_mode == "individual":
                        ...
                        if interactive:
                            print("   ✅ Added to combined output")
        ...
        if cp.is_complete:
            ...
            if cp.failed_files:
                ...
                if retry_checkpoint and interactive:
                    ...
                    print("   Run File Processor again to retry failed files.")
    ...
    def _preprocess_audio_if_needed(
        ...
        if preprocess_type == "amplify":
            ...
            if normalize_result.success:
                if interactive:
                    print("   ✅ Audio amplified and normalized")
...
def show_tools_menu() -> bool:
    ...
    if choice == "1":
        ...
        from src.gui.tts_tool import get_instance as get_tts_instance
